...
class AbstractButton {
 ...
 public:
  ...
  void (*ActionBegin)(Uint32*, int, int, int, int, int, int, int, int, int, int,
                      Uint32) = nullptr;
  void (*ActionEnd)(Uint32*, int, int, int, int, int, int, int, int, int, int,
                    int, int, Uint32) = nullptr;
  void (*ActionUpdate)(Uint32*, int, int, int, int, int, int, int, int, int,
                       int, int, int, Uint32) = nullptr;

  void (*ActionUpdateCreator)(Uint32*, int, int, int, int, int, int, int,
                              int) = nullptr;
  void (*ActionEndCreator)(AbstractButton*, int, int, int, int,
                           Uint32*) = nullptr;

  bool using_ = false;

  AbstractButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
  ...
  void MouseOn() {
    ...
    mouse_on =
        (x - X_START_WINDOW >= x_ && x - X_START_WINDOW <= x_ + length_ &&
         y - Y_START_WINDOW >= y_ && y - Y_START_WINDOW <= y_ + height_);
  }
  ...
  virtual void Use() { std::cout << "function is not assigned"; }
  ...
  void Exit() { is_hide = true; }
  int GetX() { return x_; }
  int GetY() { return y_; }
  int GetLen() { return length_; }
  int GetHei() { return height_; }
  std::string GetStr() { return path_img_; }
  bool IsHide() { return is_hide; }
  void DoHide() { is_hide = true; }
  void DoNoHide() { is_hide = false; }
