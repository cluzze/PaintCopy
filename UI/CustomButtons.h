...
class HelpButton : public AbstractButton {
 ...
 public:
  HelpButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
             int height = DEFAULT_HEI, std::string path_img = "")
      : AbstractButton(x, y, length, height, path_img),
        help_wind(GetX(), GetY() + GetHei(), 400, 30, "Img/help_window.bmp") {}
  virtual void Use() {
...
class FileButton : public AbstractButton {
 public:
  FileButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
             int height = DEFAULT_HEI, std::string path_img = "")
      : AbstractButton(x, y, length, height, path_img) {}
  virtual void Use() { std::cout << "File "; }
};

class ImageButton : public AbstractButton {
 public:
  using AbstractButton::AbstractButton;
  virtual void Use() { std::cout << "Image "; }
};
...
class ColorButton : public AbstractButton {
 ...
 public:
  ColorButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
              int height = DEFAULT_HEI, std::string path_img = "",
              ColorRGB color = {0, 0, 0}, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    color_ = color;
    brush = brush_;
  }
...
class UpButton : public AbstractButton {
 ...
 public:
  UpButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
           int height = DEFAULT_HEI, std::string path_img = "",
           Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    brush = brush_;
  }
  virtual void Use() { brush->SizeIncrease(); }
};
...
class DownButton : public AbstractButton {
 ...
 public:
  DownButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
             int height = DEFAULT_HEI, std::string path_img = "",
             Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    brush = brush_;
