...
class CustomTexture : public AbstractButton {
 ...
 public:
  CustomTexture(int x = 0, int y = 0, int length = DEFAULT_LEN,
                int height = DEFAULT_HEI, Uint32* tempPixels_ = nullptr)
  ...
  void SetPixels(Uint32* tempPixels_, int hei) {
    ...
    /*if (tempPixels_) {
            ...
            Amask = 0xff000000;
#endif
            SDL_Surface* tempSurface = SDL_CreateRGBSurface(0, GetLen(),
GetHei(), 32, Rmask, Gmask, Bmask, Amask); pTexture =
SDL_CreateTextureFromSurface(ren, tempSurface); SDL_FreeSurface(tempSurface);
