...
class ITexture {
 public:
  ...
  virtual void DrawLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
                        Color color) = 0;
  virtual void DrawThickLine(int32_t x0, int32_t y0, int32_t x1, int32_t y1,
                             int32_t thickness, Color color) = 0;
  virtual void DrawCircle(int32_t x, int32_t y, int32_t radius,
                          Color color) = 0;
  virtual void DrawRect(int32_t x, int32_t y, int32_t width, int32_t height,
                        Color color) = 0;

  virtual void CopyTexture(ITexture* texture, int32_t x, int32_t y,
...
class ITool {
 public:
  ...
  virtual void ActionBegin(unsigned __int32* tempPixels, int x_click,
                           int y_click, int len, int hei, int get_x, int get_y,
                           int show_rect_x, int show_rect_y, int scale,
                           int size, unsigned __int32 pixelColor) = 0;
  virtual void ActionUpdate(unsigned __int32* tempPixels, int x_start,
                            int y_start, int x_finish, int y_finish, int len,
                            int hei, int get_x, int get_y, int show_rect_x,
                            int show_rect_y, int scale, int size,
                            unsigned __int32 pixelColor) = 0;
  virtual void ActionEnd(unsigned __int32* tempPixels, int x_start, int y_start,
                         int x_finish, int y_finish, int len, int hei,
                         int get_x, int get_y, int show_rect_x, int show_rect_y,
                         int scale, int size, unsigned __int32 pixelColor) = 0;
};
...
class ITexture {
 public:
  ...
  virtual void CopyTexture(ITexture* source, int32_t x, int32_t y,
                           int32_t size_x, int32_t size_y) = 0;
...
class IWidgetFactory {
 public:
  ...
  virtual IButton* CreateButtonWithIcon(int32_t size_x, int32_t size_y,
                                        const char* icon_file_name) = 0;
  virtual IButton* CreateButtonWithText(int32_t size_x, int32_t size_y,
                                        const char* text,
                                        int32_t char_size) = 0;

  virtual ISlider* CreateDefaultSlider(float range_min, float range_max) = 0;
  virtual ISlider* CreateSlider(int32_t size_x, int32_t size_y, float range_min,
                                float range_max) = 0;

  virtual ILabel* CreateDefaultLabel(const char* text) = 0;
  virtual ILabel* CreateLabel(int32_t size_x, int32_t size_y, const char* text,
                              int32_t char_size) = 0;
