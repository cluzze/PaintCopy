...
bool init() {
  ...
  win = SDL_CreateWindow("Graphics", SDL_WINDOWPOS_UNDEFINED,
...
void SetColor(ColorRGB color) {
  SDL_SetRenderDrawColor(ren, (int)(color.GetR() * 255),
...
void ShowLineSprite(int start_x, int start_y, int end_x, int end_y,
                    std::string str) {
...
Uint32 ConvertColor(SDL_Color _color) {
  ...
  Uint32 pixelColor =
      (_color.b << 24) + (_color.g << 16) + (_color.r << 8) + 255;
#else
  Uint32 pixelColor =
      (255 << 24) + (_color.r << 16) + (_color.g << 8) + _color.b;
#endif
  return pixelColor;
}

SDL_Texture* GetAreaTextrue(SDL_Rect rect, SDL_Renderer* renderer,
                            SDL_Texture* source) {
  SDL_Texture* result =
