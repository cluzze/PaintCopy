...
class SizeWind : public AbstractWindow {
 ...
 public:
  SizeWind(int x = 0, int y = 0, int length = 0, int height = 0,
           Brush* brush_ = nullptr)
      : AbstractWindow(x, y, length, height),
        up_button(x + (int)(PART_WINDOW * length), y,
                  (int)(length * PART_LENGHT), (int)(length * PART_LENGHT),
                  "Img/up_button.bmp", brush_),

        down_buttom(x + (int)(PART_WINDOW * length),
                    (int)(y + length * PART_LENGHT + PART_INDENT_SIZE_WIND),
                    (int)(length * PART_LENGHT), (int)(length * PART_LENGHT),
                    "Img/down_button.bmp", brush_),

        value_wind(x + (int)(PART_INDENT_SIZE_WIND * length), y,
                   (int)(length * 0.5),
                   (int)(length * (PART_LENGHT * 2 + PART_INDENT_SIZE_WIND)),
                   "Img/bg_value.bmp") {
    brush = brush_;
    std::string str = "Img/";
    str += (char)(brush->GetSize() + INT_VALUE_0);
    str += ".bmp";
    value_button = {x + (int)(PART_INDENT_SIZE_WIND * length) + INDENT_PIX,
                    y + INDENT_PIX, (int)(length * 0.5) - INDENT_PIX * 2,
                    (int)(length * (PART_LENGHT * 2 + PART_INDENT_SIZE_WIND)) -
                        INDENT_PIX * 2,
                    str};
  }
...
class PaintButton : public AbstractButton {
 ...
 public:
  PaintButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
              int height = DEFAULT_HEI, std::string path_img = "",
              SizeWind* wind = nullptr, Brush* brush_ = nullptr)
  ...
  virtual void Use() {
    if (brush->tool_builtin) brush->tool_builtin->using_ = false;
    if (brush->tool_out) brush->tool_out->using_ = false;
    if (brush->tool_creator) brush->tool_creator->using_ = false;
    using_ = true;
...
class RectButton : public AbstractButton {
 ...
 public:
  RectButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
             int height = DEFAULT_HEI, std::string path_img = "",
             SizeWind* wind = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    size_wind = wind;
...
class FillRectButton : public AbstractButton {
 ...
 public:
  FillRectButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
                 int height = DEFAULT_HEI, std::string path_img = "",
                 SizeWind* wind = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    size_wind = wind;
...
class CircleButton : public AbstractButton {
 ...
 public:
  CircleButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
               int height = DEFAULT_HEI, std::string path_img = "",
               SizeWind* wind = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    size_wind = wind;
...
class FillCircleButton : public AbstractButton {
 ...
 public:
  FillCircleButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
                   int height = DEFAULT_HEI, std::string path_img = "",
                   SizeWind* wind = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    size_wind = wind;
...
class LineButton : public AbstractButton {
 ...
 public:
  LineButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
             int height = DEFAULT_HEI, std::string path_img = "",
             SizeWind* wind = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    size_wind = wind;
...
class FillButton : public AbstractButton {
 ...
 public:
  FillButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
             int height = DEFAULT_HEI, std::string path_img = "",
             SizeWind* wind = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    size_wind = wind;
...
class ExcretionButton : public AbstractButton {
 ...
 public:
  ExcretionButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
                  int height = DEFAULT_HEI, std::string path_img = "",
                  SizeWind* wind = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    size_wind = wind;
...
class ColorWindow : public AbstractWindow {
 ...
 public:
  ColorWindow(int x = 0, int y = 0, int length = 0, int height = 0,
              std::string path_img = "", Brush* brush_ = nullptr)
      : AbstractWindow(x, y, length, height),
        color_butR(x + (int)(PART_INDENT_COLOR_WIND * length - 2),
                   (int)(y + PART_INDENT_COLOR_WIND / 2 * height),
                   (int)(length * PART_BUTTON_COLOR_WIND / 2),
                   (int)(length * PART_BUTTON_COLOR_WIND / 2),
                   "Img/framecolor_wind.bmp", {255, 0, 0}, brush_),

        color_butG(
            x + (int)((PART_INDENT_COLOR_WIND + PART_BUTTON_COLOR_WIND / 2) *
                          length +
                      2),
            (int)(y + PART_INDENT_COLOR_WIND / 2 * height),
            (int)(length * PART_BUTTON_COLOR_WIND / 2),
            (int)(length * PART_BUTTON_COLOR_WIND / 2),
            "Img/framecolor_wind.bmp", {0, 255, 0}, brush_),

        color_butB(x + (int)(PART_INDENT_COLOR_WIND * length - 2),
                   (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                         length * PART_BUTTON_COLOR_WIND / 2 + 4),
                   (int)(length * PART_BUTTON_COLOR_WIND / 2),
                   (int)(length * PART_BUTTON_COLOR_WIND / 2),
                   "Img/framecolor_wind.bmp", {0, 0, 255}, brush_),
        ...
        color_butY(
            ...
            (int)(length * PART_BUTTON_COLOR_WIND / 2),
            (int)(length * PART_BUTTON_COLOR_WIND / 2),
            "Img/framecolor_wind.bmp", {255, 255, 0}, brush_),

        color_butW(x + (int)(PART_INDENT_COLOR_WIND * length - 2),
                   (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                         (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 2),
                   (int)(length * PART_BUTTON_COLOR_WIND / 2),
                   (int)(length * PART_BUTTON_COLOR_WIND / 2),
                   "Img/framecolor_wind.bmp", {255, 255, 255}, brush_),
        ...
        color_butBL(
            ...
            (int)(length * PART_BUTTON_COLOR_WIND / 2),
            (int)(length * PART_BUTTON_COLOR_WIND / 2),
            "Img/framecolor_wind.bmp", {0, 0, 0}, brush_),

        size_wind(x + (int)(PART_INDENT_COLOR_WIND / 2 * length),
                  (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                        (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 10 +
                        length * 0.5),
                  (int)(length * 0.9), (int)(length * 0.9), brush_),

        paint_button(x + (int)(PART_INDENT_COLOR_WIND / 2 * length),
                     (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                           (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 5),
                     (int)(length * 0.8), (int)(length * 0.5),
                     "Img/paint_button.bmp", &size_wind, brush_),

        rect_button(x + (int)((PART_INDENT_COLOR_WIND / 2 + 0.15) * length),
                    (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                          (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 15 +
                          length * 1.25),
                    (int)(length * 0.55), (int)(length * 0.35),
                    "Img/rect_button.bmp", &size_wind, brush_),

        fill_rect_button(
            x + (int)((PART_INDENT_COLOR_WIND / 2 + 0.15) * length),
            (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                  (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 20 +
                  length * 1.7),
            (int)(length * 0.55), (int)(length * 0.35),
            "Img/fillrect_button.bmp", &size_wind, brush_),

        circle_button(x + (int)((PART_INDENT_COLOR_WIND / 2 + 0.15) * length),
                      (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                            (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 25 +
                            length * 2.15),
                      (int)(length * 0.55), (int)(length * 0.35),
                      "Img/circle_button.bmp", &size_wind, brush_),

        fill_circle_button(
            x + (int)((PART_INDENT_COLOR_WIND / 2 + 0.15) * length),
            (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                  (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 30 +
                  length * 2.6),
            (int)(length * 0.55), (int)(length * 0.35),
            "Img/fillcircle_button.bmp", &size_wind, brush_),

        line_button(x + (int)((PART_INDENT_COLOR_WIND / 2 + 0.1) * length),
                    (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                          (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 35 +
                          length * 3.05),
                    (int)(length * 0.65), (int)(length * 0.45),
                    "Img/line_button.bmp", &size_wind, brush_),

        fill_button(x + (int)((PART_INDENT_COLOR_WIND / 2 + 0.1) * length),
                    (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                          (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 40 +
                          length * 3.5),
                    (int)(length * 0.65), (int)(length * 0.45),
                    "Img/fill_button.bmp", &size_wind, brush_),

        excr_button(x + (int)((PART_INDENT_COLOR_WIND / 2 + 0.1) * length),
                    (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                          (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 45 +
                          length * 3.95),
                    (int)(length * 0.65), (int)(length * 0.5),
                    "Img/excretion_button.bmp", &size_wind, brush_) {
    path_img_ = path_img;

    free_x = x + (int)((PART_INDENT_COLOR_WIND / 2 + 0.15) * length);
    free_y = (int)(y + PART_INDENT_COLOR_WIND / 2 * height +
                   (length * PART_BUTTON_COLOR_WIND / 2 + 4) * 3 + 50 +
                   length * 4.45);
  ...
  UserButton* PlaceNewButton(ITool* tool, std::string path_img) {
    if (user_buttons.size() < 4) {
      UserButton new_button(free_x, free_y, defolt_len, defolt_hei,
                            path_img.c_str(), brush, tool);
...
class CustomWindow : public AbstractButton {
 ...
 public:
  CustomWindow(int x = 0, int y = 0, int length = 0, int height = 0,
               std::string path_img = "", Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img),
        up_frame(GetX(), GetY() - DEFAULT_BUTTON_HEI, GetLen() + 15, 26,
                 "Img/bg_line.bmp", this),
        close_button(GetX() + GetLen() - 10, GetY() - 25, 25, 18,
                     "Img/close_button.bmp", this),
        slider_manager_vert(GetX() + GetLen(), GetY() + 15, 15, GetHei() - 30,
                            "Img/bgcolor_wind.bmp", scale),
        slider_manager_horiz(GetX() + 15, GetY() + GetHei(), GetLen() - 30, 15,
                             "Img/bg_slider_horizontal.bmp", scale),
        custom_texture(GetX() + cus_tex_x, GetY() + cus_tex_y, cus_tex_len,
                       cus_tex_hei, nullptr) {
    path_img_ = path_img;
    brush = brush_;
    ...
    Amask = 0xff000000;
#endif
    SDL_Surface* tempSurface = SDL_CreateRGBSurface(0, GetLen(), GetHei(), 32,
    ...
    memset(tempPixels, ConvertColor({255, 255, 255}),
           GetLen() * GetHei() * sizeof(Uint32));
    memset(old_tempPixels, ConvertColor({255, 255, 255}),
           GetLen() * GetHei() * sizeof(Uint32));
    ...
    if (!IsHide()) {
      ...
      if (brush->tool_builtin) {
        if (brush->tool_builtin->ActionBegin) {
          ...
          brush->tool_builtin->ActionBegin(tempPixels, x_click, y_click,
                                           GetLen(), GetHei(), GetX(), GetY(),
                                           show_rect.x, show_rect.y, (int)scale,
                                           brush->GetSize(), pixelColor);
        }
      }
      ...
      if (brush->tool) {
        ...
        SetPartTexture();
        CopyTempPix(old_tempPixels, tempPixels);
        brush->tool->ActionBegin(
            tempPixels, x_click, y_click, GetLen(), GetHei(), GetX(), GetY(),
            show_rect.x, show_rect.y, (int)scale, brush->GetSize(), pixelColor);
      }

      if (brush->tool_creator) {
        custom_texture.DoHide();
        SetPartTexture();
        // brush->tool->ActionBegin(tempPixels, x_click, y_click, GetLen(),
        // GetHei(), 	GetX(), GetY(), show_rect.x, show_rect.y, (int)scale,
        //brush->GetSize(), pixelColor);
      }
    ...
    if (click) {
      ...
      if (brush->tool_builtin) {
        if (brush->tool_builtin->ActionEnd) {
          brush->tool_builtin->ActionEnd(
              tempPixels, x_start, y_start, x_finish, y_finish, GetLen(),
              GetHei(), GetX(), GetY(), show_rect.x, show_rect.y, (int)scale,
              brush->GetSize(), pixelColor);
        }
      }
      if (brush->tool) {
        brush->tool->ActionEnd(tempPixels, x_start, y_start, x_finish, y_finish,
                               GetLen(), GetHei(), GetX(), GetY(), show_rect.x,
                               show_rect.y, (int)scale, brush->GetSize(),
                               pixelColor);
      }
      if (brush->tool_creator) {
        brush->tool_creator->ActionEndCreator(
            this, cus_tex_x, cus_tex_y, cus_tex_len, cus_tex_hei, nullptr);
        custom_texture.DoNoHide();
        Uint32* ParttempPixels;

        ParttempPixels =
            new Uint32[custom_texture.GetLen() * custom_texture.GetHei()];

        for (int i = custom_texture.GetX();
             i < custom_texture.GetX() + custom_texture.GetLen(); i++) {
          for (int j = custom_texture.GetY() - GetY();
               j < custom_texture.GetY() + custom_texture.GetHei() - GetY();
               j++) {
            ParttempPixels[i - custom_texture.GetX() +
                           custom_texture.GetLen() *
                               (j - (custom_texture.GetY() - GetY()))] =
                tempPixels[i + j * GetLen()];
        ...
        // custom_texture.SetAll(cus_tex_x + GetX(), cus_tex_y + GetY(),
        // cus_tex_len, cus_tex_hei, ParttempPixels);
      }
      SDL_UpdateTexture(pTexture, NULL, tempPixels, GetHei() * sizeof(Uint32));
    }
    click = false;
  ...
  void Update() {
      ...
      if (click) {
        ...
        if (brush->tool_builtin) {
          if (brush->tool_builtin->ActionUpdate) {
            brush->tool_builtin->ActionUpdate(
                tempPixels, x_click, y_click, x_finish, y_finish, GetLen(),
                GetHei(), GetX(), GetY(), show_rect.x, show_rect.y, (int)scale,
                brush->GetSize(), pixelColor);
          }
        }
        if (brush->tool) {
          brush->tool->ActionUpdate(tempPixels, x_click, y_click, x_finish,
                                    y_finish, GetLen(), GetHei(), GetX(),
                                    GetY(), show_rect.x, show_rect.y,
                                    (int)scale, brush->GetSize(), pixelColor);
        }
        if (brush->tool_creator) {
          brush->tool_creator->ActionUpdateCreator(tempPixels, x_click, y_click,
                                                   x_finish, y_finish, GetLen(),
                                                   GetHei(), GetX(), GetY());
          cus_tex_x = x_click;
          cus_tex_y = y_click;
        ...
        SDL_UpdateTexture(pTexture, NULL, tempPixels,
                          GetHei() * sizeof(Uint32));
      }
    ...
    if (IsChangePos()) {
      ...
      custom_texture.Moving(GetX() + cus_tex_x - x_start_change,
                            GetY() + cus_tex_y - y_start_change, cus_tex_len,
                            cus_tex_hei);
    }
      ...
      for (int i = custom_texture.GetX();
        ...
        for (int j = custom_texture.GetY() - GetY();
          ...
          if (custom_texture.GetPixels())
            tempPixels[i + j * GetLen()] =
                custom_texture
                    .GetPixels()[i - custom_texture.GetX() +
                                 custom_texture.GetLen() *
                                     (j - (custom_texture.GetY() - GetY()))];
        }
      }
  ...
  virtual bool UpdateEvent(SDL_Event* event) {
    ...
    if (!IsHide()) {
      ...
      if (slider_manager_vert.UpdateEvent(event)) {
        ok = true;
      }
      if (slider_manager_horiz.UpdateEvent(event)) {
        ok = true;
      }
      if (up_frame.UpdateEvent(event)) {
        ok = true;
      }
  ...
  void CreatExcretion(int x, int y, int len, int hei, Uint32* tempPixels) {
    ...
    cus_tex_len = len;
    cus_tex_hei = hei;
    custom_texture.SetAll(cus_tex_x, cus_tex_y, cus_tex_len, cus_tex_hei,
                          tempPixels);
  }
};

void ActionEndExcretion(AbstractButton* cust_wind, int x, int y, int len,
                        int hei, Uint32* tempPixels) {
  static_cast<CustomWindow*>(cust_wind)->CreatExcretion(x, y, len, hei,
                                                        tempPixels);
}
...
class ButtonConectCustomWind : public AbstractButton {
 ...
 public:
  ButtonConectCustomWind(int x = 0, int y = 0, int length = 0, int height = 0,
                         std::string path_img = "",
                         CustomWindow** wind = nullptr, int numb = 0)
      : AbstractButton(x, y, length, height, path_img) {
    wind_ = wind;
    numb_of_wind = numb;
  }
...
class WindowWind : public AbstractButton {
 ...
 public:
  WindowWind(int x = 0, int y = 0, int length = 0, int height = 0,
             std::string path_img = "", CustomWindow** wind = nullptr,
             int numb = 0)
      : AbstractButton(x, y, length, height, path_img),
        button_custom_wind(x, y, length, height, "Img/show_button.bmp", wind,
                           numb) {
    path_img_ = path_img;
    is_hide = true;
  }
...
class WindowButton : public AbstractButton {
 ...
 public:
  WindowButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
               int height = DEFAULT_HEI, std::string path_img = "",
               CustomWindow** wind = nullptr, int numb = 0)
      : AbstractButton(x, y, length, height, path_img),
        image_wind(GetX(), GetY() + GetHei(), 40, DEFAULT_BUTTON_HEI,
                   "Img/bg_wind.bmp", wind, numb) {}
  virtual void Show() {
    TailShow();
    image_wind.Show();
  }
...
class SinglePluginButton : public AbstractButton {
 ...
 public:
  SinglePluginButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
                     int height = DEFAULT_HEI, std::string path_img = "",
                     std::string button_text = "",
                     ColorWindow* color_window = nullptr,
                     CustomWindow* cust_wind_ = nullptr,
                     Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img),
        text(ren, "Text/arial.ttf", 16, button_text, {150, 180, 180, 255}) {
  ...
  virtual void Show() { text.display(GetX(), GetY(), ren); }
  virtual void Use() {
    ...
    if (hDLL) {
      ITool* (*factory_func)() =
          ((ITool * (*)()) GetProcAddress(hDLL, "create_tool"));
      ITool* instance = factory_func();

      IWidgetFactory* (*widget_func)() =
          ((IWidgetFactory * (*)()) GetProcAddress(hDLL, "create_widget"));
      IWidgetFactory* widget_instance = widget_func();

      user_but =
          color_wind->PlaceNewButton(instance, widget_instance->GetPath());
    }
  }
};
...
class PluginWindow : public AbstractButton {
 ...
 public:
  PluginWindow(int x = 0, int y = 0, int length = 0, int height = 0,
               std::string path_img = "",
               std::vector<std::string>* plugins_names_ = nullptr,
               ColorWindow* color_window = nullptr,
               CustomWindow* cust_wind_ = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    if (plugins_names_) {
      SetHei(height * plugins_names_->size());
      for (unsigned int i = 0; i < plugins_names_->size(); i++) {
        SinglePluginButton plugin_button(
            GetX() + 5, GetY() + GetHei() / plugins_names_->size() * i + 5,
            GetLen(), GetHei() / plugins_names_->size(),
            "Img/framecolor_wind.bmp", (*plugins_names_)[i], color_window,
            cust_wind_, brush_);
...
class PluginButton : public AbstractButton {
 ...
 public:
  PluginButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
               int height = DEFAULT_HEI, std::string path_img = "",
               std::vector<std::string>* plugins_names_ = nullptr,
               ColorWindow* color_window = nullptr,
               CustomWindow* cust_wind_ = nullptr, Brush* brush_ = nullptr)
      : AbstractButton(x, y, length, height, path_img),
        plugin_window(GetX(), GetY() + GetHei(), 200, 30, "Img/bg_wind.bmp",
                      plugins_names_, color_window, cust_wind_, brush_) {}
  virtual void Use() {
  ...
  virtual bool UpdateEvent(SDL_Event* event) {
    ...
    if (!plugin_window.IsHide()) plugin_window.UpdateEvent(event);
    return false;
  }
};
...
class MainWindow : public AbstractWindow {
 private:
  CloseButton close_button{SCREEN_WIDTH - 25,      0,   25, 18,
                           "Img/close_button.bmp", this};
  SimpleButton simple_button{SCREEN_WIDTH - 50, 0, 25, 18,
                             "Img/hide_button.bmp"};
  ...
  ImageButton image_button{45, 0, 90, DEFAULT_BUTTON_HEI,
                           "Img/image_button.bmp"};
 ...
 public:
  MainWindow(std::vector<std::string>* plugins_name_ = nullptr, int x = 0,
             int y = 0, int length = 0, int height = 0)
      : AbstractWindow(x, y, length, height),
        color_wind(45, 45, 45, 420, "Img/bgcolor_wind.bmp", &brush),
    ...
    window_button = {
        190,         0, 45, DEFAULT_BUTTON_HEI, "Img/window_button.bmp",
        custom_wind, 2};
    plagin_button = {235,
                     0,
                     45,
                     DEFAULT_BUTTON_HEI,
                     "Img/plugin_button.bmp",
                     plugins_name_,
                     &color_wind,
                     &custom_wind1,
                     &brush};
  ...
  void Show() {
    if (!IsHide()) {
      ...
      color_wind.Show();
      if (custom_wind[0]) custom_wind[0]->Show();
      if (custom_wind[1]) custom_wind[1]->Show();

      plagin_button.Show();
      texture.Show();
    }
  }
  virtual void Exit() { DoHide(); }
