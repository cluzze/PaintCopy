...
class SimpleButton : public AbstractButton {
 public:
  ...
  virtual void Use() { std::cout << "Simple "; }
};
...
class CloseButton : public AbstractButton {
 ...
 public:
  CloseButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
              int height = DEFAULT_HEI, std::string path_img = "",
              AbstractWindow* parent = nullptr)
...
class CloseButtonBut : public AbstractButton {
 ...
 public:
  CloseButtonBut(int x = 0, int y = 0, int length = DEFAULT_LEN,
                 int height = DEFAULT_HEI, std::string path_img = "",
                 AbstractButton* parent = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    parent_ = parent;
  }
  virtual void Use() {
...
class UpFrame : public AbstractButton {
 ...
 public:
  UpFrame(int x = 0, int y = 0, int length = DEFAULT_LEN,
          int height = DEFAULT_HEI, std::string path_img = "",
          AbstractButton* parent_ = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    parent = parent_;
  }
  virtual void Show() {
    ShowLineSprite(GetX(), GetY(), GetX() + GetLen(), GetY() + GetHei(),
                   GetStr());
  }
  virtual void Use() { parent->DoChangePos(); }
  virtual void NoUse() { parent->StopChangePos(); }
};
...
class SliderVertical : public AbstractButton {
 ...
 public:
  SliderVertical(int x = 0, int y = 0, int length = DEFAULT_LEN,
                 int height = DEFAULT_HEI, std::string path_img = "",
                 AbstractButton* slider_manager = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
  ...
  void Update() {
    if (click) {
      ...
      y_click = std::min(
          slider_manager_->GetY() + slider_manager_->GetHei() - GetHei() - 3,
          y_click);
      SetY(y_click);
    }
  }
  virtual void NoUse() { click = false; }
...
class SliderHorizontal : public AbstractButton {
 ...
 public:
  SliderHorizontal(int x = 0, int y = 0, int length = DEFAULT_LEN,
                   int height = DEFAULT_HEI, std::string path_img = "",
                   AbstractButton* slider_manager = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    slider_manager_ = slider_manager;
  ...
  void Update() {
    if (click) {
      ...
      x_click = std::min(
          slider_manager_->GetX() + slider_manager_->GetLen() - GetLen() - 3,
          x_click);
      SetX(x_click);
    }
  }
...
class UpButtonSlider : public AbstractButton {
 ...
 public:
  UpButtonSlider(int x = 0, int y = 0, int length = DEFAULT_LEN,
                 int height = DEFAULT_HEI, std::string path_img = "",
                 SliderVertical* slider = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    slider_ = slider;
    click = false;
  }
...
class RightButtonSlider : public AbstractButton {
 ...
 public:
  RightButtonSlider(int x = 0, int y = 0, int length = DEFAULT_LEN,
                    int height = DEFAULT_HEI, std::string path_img = "",
                    SliderHorizontal* slider = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    slider_ = slider;
    click = false;
  }
  void Use() {
    int new_pos =
        std::min(slider_->GetX() + 10, GetX() - slider_->GetLen() - 3);
...
class LeftButtonSlider : public AbstractButton {
 ...
 public:
  LeftButtonSlider(int x = 0, int y = 0, int length = DEFAULT_LEN,
                   int height = DEFAULT_HEI, std::string path_img = "",
                   SliderHorizontal* slider = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    slider_ = slider;
...
class DownButtonSlider : public AbstractButton {
 ...
 public:
  DownButtonSlider(int x = 0, int y = 0, int length = DEFAULT_LEN,
                   int height = DEFAULT_HEI, std::string path_img = "",
                   SliderVertical* slider = nullptr)
      : AbstractButton(x, y, length, height, path_img) {
    slider_ = slider;
  ...
  void Use() {
    int new_pos =
        std::min(slider_->GetY() + 10, GetY() - slider_->GetHei() - 3);
    slider_->SetY(new_pos);
    click = true;
  }
  void Update() {
...
class SliderManagerVertical : public AbstractButton {
 ...
 public:
  SliderManagerVertical(int x = 0, int y = 0, int length = DEFAULT_LEN,
                        int height = DEFAULT_HEI, std::string path_img = "",
                        double scale = 1)
      : AbstractButton(x, y, length, height, path_img),
        slider(x + 3, y + 3, length - 6, (int)(height / scale),
               "Img/slider_button.bmp", this),
        up_button{x, y - length, length, length, "Img/up_button.bmp", &slider},
        down_button{x,      y + height, length, length, "Img/down_button.bmp",
                    &slider} {
    scale_ = scale;
  }
  virtual void Show() {
  ...
  virtual bool UpdateEvent(SDL_Event* event) {
    bool ok = false;
    if (slider.UpdateEvent(event)) {
      ok = true;
    }
    if (up_button.UpdateEvent(event)) {
      ok = true;
    }
    if (down_button.UpdateEvent(event)) {
      ok = true;
    }
    return ok;
  }
  ...
  int GetSliderRelativePos() { return slider.GetY() - GetY(); }
  UpButtonSlider* UpClick() { return &up_button; }
  DownButtonSlider* DownClick() { return &down_button; }
...
class SliderManagerHorizontal : public AbstractButton {
 ...
 public:
  SliderManagerHorizontal(int x = 0, int y = 0, int length = DEFAULT_LEN,
                          int height = DEFAULT_HEI, std::string path_img = "",
                          double scale = 1)
      : AbstractButton(x, y, length, height, path_img),
        slider(x + 3, y + 3, (int)(length / scale), height - 6,
               "Img/slider_button.bmp", this),
        right_button{x + length, y, height, height, "Img/right_button.bmp",
                     &slider},
        left_button{x - height, y, height, height, "Img/left_button.bmp",
                    &slider} {
    scale_ = scale;
  }
  virtual void Show() {
  ...
  virtual bool UpdateEvent(SDL_Event* event) {
    ...
    if (right_button.UpdateEvent(event)) {
      ok = true;
    }
    if (left_button.UpdateEvent(event)) {
      ok = true;
    }
    return ok;
  }
  ...
  int GetSliderRelativePos() { return slider.GetX() - GetX(); }

  RightButtonSlider* RightClick() { return &right_button; }
  LeftButtonSlider* LeftClick() { return &left_button; }
  void ChangeSlider(double scale) {
    scale_ = scale;
...
class UserButton : public AbstractButton {
 ...
 public:
  UserButton(int x = 0, int y = 0, int length = DEFAULT_LEN,
             int height = DEFAULT_HEI, std::string path_img = "",
             Brush* brush_ = nullptr, ITool* tool_ = nullptr)
